...
<div class="bg-white rounded-xl border p-4 relative group">
   <img 
    src="{{ asset('storage/' . $qr->qr_image_url) }}" 
    alt="{{ $qr->title }}"
...
/>
    ...
    <div class="flex gap-2 mt-4">
        <!-- VIEW BUTTON - Data attributes mein sab store karo -->
        <button type="button"
...
document.getElementById('modalDownloadLink').href = fullImageUrl;
    ...
    if (status === 'Active') {
        ...
        statusElem.className = 'px-3 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800';
    } else {
        statusElem.textContent = 'Inactive';
        statusElem.className = 'px-3 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800';
    }
    ...
    if (mode === 'dynamic' && shortUrl) {
        ...
        const fullUrl = `${window.location.origin}/qr/${shortUrl}`;
        document.getElementById('modalShortUrl').value = fullUrl;
    } else {
